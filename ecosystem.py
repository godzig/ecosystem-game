...
class Game(object):
    ...
    def __init__(self, id, players=3):
        self.cardset = set(['bear', 'bee', 'meadow', 'trout', 'eagle',
                            'rabbit', 'dragonfly', 'fox', 'deer', 'stream',
                            'wolves'])
        self.deck = ['bear'] * 12 + \
            8 * ['bee'] + \
            20 * ['meadow'] + \
    ...
    def _deerLines(self, board, positions):
        rows = [set([0, 1, 2, 3, 4]), set([5, 6, 7, 8, 9]),
                set([10, 11, 12, 13, 14]), set([15, 16, 17, 18, 19])]
        cols = [set([0, 5, 10, 15]), set([1, 6, 11, 16]),
                set([2, 7, 12, 17]), set([3, 8, 13, 18]), set([4, 9, 14, 19])]
        count = 0
        for row in rows:
